...
namespace EuclidStone
{
    ...
    namespace Polygons
    {
        public abstract class IPolygon
        {
            ...
            {
                return this.Edges().Trail(2, true).Sum(a => a[0].X * a[1].Y - a[0].Y * a[1].X) / 2;
            }
            ...
            {
                return this.Edges().Trail(2, true).Select(a => new LineSegment(a[0], a[1]));
            }
            ...
            public virtual bool isWithin(PointF point)
            {
                ...
                return this.Segments().Where(segment => r.intersect(segment).HasValue).Aggregate(false, (current, segment) => !current);
            }
